...
public class FoxHoundGame {
    ...
    private static void gameLoop(int dim, String[] players) {
        ...
        while(!exit) {
            ...
            String[] play = FoxHoundUI.positionQuery(dim, STDIN_SCAN);
            boolean valid = FoxHoundUtils.isValidMove(dim,players,turn,play[0],play[1]);
            ...
            switch(choice) {
                case FoxHoundUI.MENU_MOVE:
                    ...
                    if(valid == true) {
                        ...
                        }else if(play[0] == players[1]) {
                            players[1] = play[1];

                        }else if(play[0] == players[2]) {
                            players[2] = play[1];

                        }else if(play[0] == players[3]) {
                            players[3] = play[1];

                        }else if(play[0] == players[4]) {
                            players[4] = play[1];
                        }
                        exit = true;
                    } else {
                        ...
                        FoxHoundUtils.isValidMove(dim,players,turn,play[0],play[1]);
                    }
                    break;
