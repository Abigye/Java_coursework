...
public class FoxHoundUI {
       ...
       /* int k = 0;
        ...
        for (int row = 0; row < dimension; row++) {
            ...
            for (int col = 0; col < dimension; col++) {
                if (players[k].charAt(0) == 'A' + col && Integer.parseInt(Character.toString(players[k].charAt(1))) == row + 1) {
    ...
    public static String[] positionQuery(int dim, Scanner test_in) {
        ...
        if (test_in.hasNext()) {
            String pos1 = test_in.next();
            String pos2 = test_in.next();


            char charOfOrigin = pos1.charAt(0);
            int numOrigin = Integer.parseInt(String.valueOf(pos1.charAt(1)));
            char charOfDest = pos2.charAt(0);
            int numDest = Integer.parseInt(String.valueOf(pos2.charAt(1)));


            validPos = ((charOfOrigin >= 'A' && charOfOrigin <= 'H') && (numOrigin >= 1 && numOrigin <= dim))
                    &&
                    ((charOfDest >= 'A' && charOfDest <= 'H') && (numDest >= 1 && numDest <= dim));
            ...
            if ((validPos)) {
                ...
                System.out.println();
                return positionQuery(dim, test_in);
            }
